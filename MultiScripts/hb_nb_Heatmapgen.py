...
def heatmap(name, project):
    ...
    for i in range(0,len(titleList)):
        for t in range(1, DockPerAntibody):
            try:
                with open(str(titleList[i])+"_balanced_model_" + str(t) + "_interaction_table.csv", "r", newline='', encoding='utf8') as firstDock:
                    firstDockReader=csv.reader(firstDock, delimiter=' ')
                    dockList = []
        ...
        #define the array to collect the desired data
                ...
                for x in range(1,len(dockList)):
                    if dockList[x][0][:2]=="A0" or dockList[x][2][:2]=="A0":
    ...
    #write to csv file
    with open(str(project) + "_Heatmap.csv", "w",newline='') as csv_file:
