...
# headerList = ['Donor AtomD Acceptor AtomA Distance C-alpha_Distance Output']

def hydrophobicBonds(file):
    fileName = file.split('.pdb')
    fileName = fileName[0]
    os.system('./hbplus -h 2.90 -d 3.90 -N ' + file)
    os.rename(fileName + '.nb2', fileName + '.txt')
    myTxt = fileName + '.txt'
    ...
    # remove spaces
    lines = [line.replace('GLN', 'GLN ') for line in lines]
    ...
    with open(fileName + '_final.csv', 'r', newline='', encoding='utf8') as unsorted:
        with open(fileName + "_nnb.csv", "w", newline='', encoding='utf8') as sorted:
